...
public class PrePostProcessor {
    ...
    private static float mThreshold = 0.7f; // score above which a detection is generated
    ...
    /**
     ...
     - limit: the maximum number of boxes that will be selected  선택될 최대 상자 수
     - threshold: used to decide whether boxes overlap too much  상자가 너무 많이 겹치는지 여부를 결정하는 데 사용됩니다.
     */
    static ArrayList<Result> nonMaxSuppression(ArrayList<Result> boxes, int limit, float threshold) {
        ...
        // The algorithm is simple: Start with the box that has the highest score.  점수가 가장 높은 상자부터 시작합니다.
        // Remove any remaining boxes that overlap it more than the given threshold   주어진 임계값보다 더 많이 겹치는 나머지 상자를 제거합니다.
        // amount. If there are any boxes left (i.e. these did not overlap with any  양. 남은 상자가 있는 경우(즉, 상자와 겹치지 않는
        // previous boxes), then repeat this procedure, until no more boxes remain  , 더 이상 상자가 남지 않을 때까지 이 절차를 반복합니다
    ...
    /**
     Computes intersection-over-union overlap between two bounding boxes.   두 경계 상자 간의 교집합 오버랩을 계산합니다.
     */
